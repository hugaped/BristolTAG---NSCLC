...
```{r}
modseq1 <- readRDS(file="JAGS_Results/NS1_PFS_fpoly1_EgyptExcl.RDS")
modseq2 <- readRDS(file="JAGS_Results/NS1_PFS_fpoly2_EgyptExcl.RDS")
...
These are shown only for the 5 candidate models, but additional ones could be specified here.
...
```{r, fig.cap="Time-varying (implied) hazard ratios for all treatments versus ABCP, estimated from selected 2nd order fractional polynomial of PFS for NS1. Shaded regions represent 95%CrIs"}
...
knitr::kable(param.df, col.names=c("Mean", "SD", "2.5%", "Median", "97.5%"),
                  digits=2, 
                  caption="Parameter estimates for selected 2nd order fractional polynomial. mu represent the study-specific baselines and d the treatment effect versus PPCT, for each fractional polynomial coefficient. Posterior quantiles (median, 2.5% and 95.5%) are shown.")
```
...
### Multivariate normal parameters for treatment effects from selected model
...
info.df <- data.frame("Title"=c("Fractional polynomial multivariate normal distribution of MCMC samples - means and covariances for key model parameters from 2nd order FP models",
...
wb <- openxlsx::createWorkbook(title="NS1_EgyptExcl")
...
openxlsx::saveWorkbook(wb=wb, file="MVN_NS1_EgyptExcl.xlsx", overwrite=TRUE)
```
