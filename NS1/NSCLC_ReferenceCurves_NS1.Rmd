---
title: "NSCLC - Reference curves for NS1"
author: "Hugo Pedder"
date: "2023-11-01"
...
wb <- openxlsx::createWorkbook(title="NS1_ReferenceCurves")
...
ipd <- read.csv("NS1 PFS IPD extraction.csv")

refstudy <- "KEYNOTE189"
ipd.ref <- subset(ipd, study==refstudy & treatment=="PPCT")

write.csv(ipd.ref, paste0("NS2_IPDreference_", refstudy, ".csv"),)
...
modseq1 <- readRDS(file="JAGS_Results/NS1_PFS_fpoly1.RDS")
modseq2 <- readRDS(file="JAGS_Results/NS1_PFS_fpoly2.RDS")
modseq.fp <- append(modseq1, modseq2)
class(modseq.fp) <- "sequence.fpoly"

modfit.fp <- summary(modseq.fp)
fp.mod <- modseq.fp[["FP_-0.5_-0.5"]]
...
```{r}
openxlsx::saveWorkbook(wb=wb, file="MVN_NS1_ReferenceCurves.xlsx", overwrite=TRUE)
```
